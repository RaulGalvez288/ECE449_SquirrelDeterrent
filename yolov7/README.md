...
## Known bugs with Testing
If you encounter :
```shell
qt.qpa.xcb: could not connect to display 
qt.qpa.plugin: Could not load the Qt platform plugin "xcb" in "/home/pi/myenv/lib/python3.11/site-packages/cv2/qt/plugins" even though it was found.
This application failed to start because no Qt platform plugin could be initialized. Reinstalling the application may fix this problem.

Available platform plugins are: xcb.
```
In order to fix it you must run:
``` shell
export DISPLAY=:0
```
Additionally, your camera must be on ```/dev/video0```. This can be checked with 
``` shell
